...
base_dir=/vendor/firmware/wifi
base_legacy_dir=/vendor/etc/wifi

fw_var=`cat /sys/module/scsc_mx/parameters/firmware_variant`
fw_suffix=`cat /sys/module/scsc_mx/parameters/firmware_hw_ver`

# Check FW image path by GKI update
if [ -f ${base_legacy_dir}/${fw_var}${fw_suffix}.bin ]; then
	base_dir=$base_legacy_dir
fi
